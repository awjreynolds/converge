...
export function defineAgentPortConformance(harness: AgentPortConformanceHarness): void {
  describe(`${harness.providerId} AgentPort conformance`, () => {
    ...
    it.each(["approved", "denied"] as const)(
      "keeps an execution %s decision separate from design approval",
      async (decision) => {
        const fixture = harness.create("approval");
        const events: AgentEvent[] = [];
        const run = (async () => {
          for await (const event of fixture.agent.run(request(harness.providerId))) {
            events.push(event);
